...
namespace GLInternal {
    ...
    template<typename Type>
    struct GLColorType<Mth::CVector<Type, 1>> {
        enum : GLenum {type = GLType<Type>::type};
        enum : GLenum {format = GL_RED};
    };
    
    template<typename Type>
    struct GLColorType<Mth::CVector<Type, 2>> {
        enum : GLenum {type = GLType<Type>::type};
        enum : GLenum {format = GL_RG};
    };
    
    template<typename Type>
    struct GLColorType<Mth::CVector<Type, 3>> {
        enum : GLenum {type = GLType<Type>::type};
        enum : GLenum {format = GL_RGB};
    };
    
    template<typename Type>
    struct GLColorType<Mth::CVector<Type, 4>> {
        enum : GLenum {type = GLType<Type>::type};
    ...
    struct GLBytePacker<Mth::CVector<Type, N>> {
        enum { size = N * GLBytePacker<Type>::size };
        static void pack(uint8_t* dst, Mth::CVector<Type, N> const& src) {
            for (int i = 0; i < N; ++i) {
